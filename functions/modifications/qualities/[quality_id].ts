...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    const results = await context.env.GENESYS.prepare(`INSERT INTO QualityModification (quality_id, type, ranks) VALUES (?1, ?2, ?3) RETURNING *`)
        .bind(context.params.quality_id, createResult.type, createResult.ranks)
        .first<Modifier>();
    return Response.json(results);
}